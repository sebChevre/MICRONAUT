...
|`inplace`
|If present, generates the project in the current directory (project nom is optional if this flag is set)
|`--inplace`
|===
...
`nom` (mandatory) and `stadium` (optional).
...
ClubsApi : @Post("/") Club save(@NotNull String nom, String stadium)
....
endif::[]
...
icon:hand-o-right[] Finally, run a Consul instance with Docker:
    ...
    $ docker run -d --nom=dev-consul -e CONSUL_BIND_INTERFACE=eth0 -e CONSUL_UI_BETA=true -p 8500:8500 consul
...
icon:hand-o-right[] First of all, run MongoDB with Docker:
    ...
    $ docker run -d --nom=dev-mongo -p 27017:27017 mongo
...
----
<1> `"clubs"` is the Consul nom for the Clubs microservice (which registers
    itself with the `micronaut.application.nom` property).
...
it with 2 string fields: `nom` and `stadium`, and its constructor, getters, etc.
...
2 HTTP calls to the `clubs` microservice, to get the nom of each clubs. It will use `ClubsClient` for that.
